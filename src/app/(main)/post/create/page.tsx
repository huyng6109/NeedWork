...
import type { PostType, SalaryCurrency } from "@/types";
...
type JobSeekingFormMode = "basic" | "detailed";
...
export default function CreatePostPage() {
  ...
  const [jobSeekingFormMode, setJobSeekingFormMode] =
    useState<JobSeekingFormMode>("detailed");
  ...
  const [salaryCurrency, setSalaryCurrency] =
    useState<SalaryCurrency>(DEFAULT_SALARY_CURRENCY);

  const submitLabel =
    postType === "job_offer" ? "Đăng tin tuyển dụng" : "Đăng bài tìm việc";
    ...
    setContactItems((items) =>
      items.map((item) =>
        item.id === contactId
          ? {
              ...item,
              [field]: field === "type" ? (nextValue as ContactType) : nextValue,
            }
          : item
      )
    );
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (postType === "job_offer") {
      ...
      content = [
        ...
        normalizedContacts.length > 0 &&
          `\nThông tin liên lạc:\n${normalizedContacts
            .map((item) => {
              const option = CONTACT_OPTIONS.find((contact) => contact.value === item.type);
    ...
    if (!response.ok) {
      ...
      toast.error(error.error ?? "Đăng bài thất bại");
      return;
    }
    ...
    <div className="mx-auto max-w-3xl pb-10">
      <div className="mb-8 flex items-center justify-between gap-4">
        ...
        <div className="flex-1 text-center">
          <h1 className="text-3xl font-bold theme-text">Tạo bài đăng mới</h1>
        ...
        <div className="card mb-8 space-y-3 p-4">
          <div>
            ...
            <p className="mt-1 text-xs theme-text-muted">
              Form cơ bản chỉ gồm tiêu đề và nội dung. Form chi tiết giữ bố cục đầy đủ
              hiện tại.
            </p>
          </div>
